function Ytest = tssvm(Ctest,Ctrain,Ytrain,varargin)
...
%     Ytest = labels((y1>0) + 1);
%    
    model = svmtrain(Ytrain, Strain', '-t 3 -c 10 -q');
    [Ytest, ~, ~] = svmpredict(ones(size(Stest,2),1), Stest', model, '-q');
%     Ytest = classify(Stest',Strain',Ytrain,'linear');
